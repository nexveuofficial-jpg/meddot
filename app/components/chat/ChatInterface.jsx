...
import { ArrowLeft, MoreVertical, Search, User, Info } from "lucide-react";
...
export default function ChatInterface({ roomId, basePath = "/chat" }) {
    ...
    const handleDirectMessage = async (targetUserId) => {
        try {
            ...
            if (error) throw error;
            if (roomId) router.push(`/messages/room/${roomId}`);
            ...
            <header className="flex-shrink-0 z-30 px-4 py-3 bg-[#0F1623]/80 backdrop-blur-xl border-b border-white/5 shadow-lg select-none">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link href={basePath} className="p-2 -ml-2 text-slate-400 hover:text-white hover:bg-white/5 rounded-full transition-all md:hidden">
                            <ArrowLeft size={22} />
                        </Link>
